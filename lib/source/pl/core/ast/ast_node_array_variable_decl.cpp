...
namespace pl::core::ast {
    ...
    std::unique_ptr<ptrn::Pattern> ASTNodeArrayVariableDecl::createStaticArray(Evaluator *evaluator) const {
        ...
        if (dynamic_cast<ptrn::PatternPadding *>(templatePattern.get())) {
            outputPattern = std::make_unique<ptrn::PatternPadding>(evaluator, startOffset, 0, getLine());
        } else if (dynamic_cast<ptrn::PatternCharacter *>(templatePattern.get())) {
            outputPattern = std::make_unique<ptrn::PatternString>(evaluator, startOffset, 0, getLine());
        } else if (dynamic_cast<ptrn::PatternWideCharacter *>(templatePattern.get())) {
            outputPattern = std::make_unique<ptrn::PatternWideString>(evaluator, startOffset, 0, getLine());
        } else {
            auto arrayPattern = std::make_unique<ptrn::PatternArrayStatic>(evaluator, startOffset, 0, getLine());
    ...
    std::unique_ptr<ptrn::Pattern> ASTNodeArrayVariableDecl::createDynamicArray(Evaluator *evaluator) const {
        ...
        evaluator->alignToByte();
        auto arrayPattern = std::make_unique<ptrn::PatternArrayDynamic>(evaluator, evaluator->getReadOffset(), 0, getLine());
