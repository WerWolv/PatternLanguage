...
namespace pl {
    ...
    public:
        ...
        [[nodiscard]] std::vector<std::unique_ptr<Pattern>> createPatterns(Evaluator *evaluator) const override {
            auto bitfieldPattern = std::make_unique<PatternBitfield>(evaluator, evaluator->dataOffset(), 0);

            size_t bitOffset = 0x00;

            std::vector<std::shared_ptr<Pattern>> fields;
            std::vector<std::shared_ptr<Pattern>> potentialPatterns;
            ...
            for (auto &entry : entries) {
                ...
                for (auto &pattern : potentialPatterns) {
                    if (auto bitfieldField = dynamic_cast<PatternBitfieldField*>(pattern.get()); bitfieldField != nullptr) {
            ...
            return pl::moveToVector<std::unique_ptr<Pattern>>(std::move(bitfieldPattern));
        }
