...
namespace pl::err {
    ...
    public:
        Error(char prefix, u32 errorCode, std::string title) :
            m_prefix(prefix), m_errorCode(errorCode), m_title(std::move(title)) {

        }

        Error(const Error &error, std::string description, std::string hint = { })
        ...
        [[noreturn]] void throwError(const std::string &description, const std::string &hint = { }) const {
            throw Error(*this, description, hint);
        }
