...
namespace pl {
    ...
    inline std::string toBinaryString(pl::unsigned_integral auto number) {
        if (number == 0) return "0";

        std::string result;
        for (i16 bit = pl::bit_width(number) - 1; bit >= 0; bit--)
...
#define SCOPE_GUARD   ::pl::scope_guard::ScopeGuardOnExit() + [&]()
